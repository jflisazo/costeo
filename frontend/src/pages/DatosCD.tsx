...
export default function DatosCD() {
  ...
  if (!init) {
    setRows(rec.estado.datos_cd.filter(d => (d.item_aux ?? 'Item') === 'Item'))
    setInit(true)
  }

  const estado = rec.estado
  const itemsObra = estado.items.filter(i => i.tipo === 'Item')
  const itemOpciones = itemsObra.map(i => ({
    value: i.numero, label: `${i.numero} — ${i.descripcion}`,
  }))
  ...
  const guardar = async () => {
    // Mantener las filas Aux existentes y reemplazar solo las Item
    const auxRows = estado.datos_cd.filter(d => d.item_aux === 'Aux')
    const todos = [...auxRows, ...rows.map(r => ({ ...r, item_aux: 'Item' as const }))]
    await patch.mutateAsync({ seccion: 'datos_cd', data: todos })
    const res = await recalc.mutateAsync()
    setRows(res.estado.datos_cd.filter(d => (d.item_aux ?? 'Item') === 'Item'))
  ...
  const cols: ColumnsType<{ r: DatoCD; i: number }> = [
    { title: 'Item/Aux', width: 90, fixed: 'left',
      render: (_, { r, i }) => (
        <Select size="small" style={{ width: 80 }} value={r.item_aux}
          onChange={v => upd(i, 'item_aux', v)}
          options={[{ value: 'Item' }, { value: 'Aux' }]} />
      ) },
    { title: 'Item', width: 220, fixed: 'left',
      render: (_, { r, i }) => (
        <Select size="small" style={{ width: 210 }} value={r.item_id || undefined}
          showSearch optionFilterProp="label" placeholder="Seleccionar ítem"
          onChange={v => upd(i, 'item_id', v)}
          options={itemOpciones} />
      ) },
  ...
  const sumCU = rows.reduce((s, r) => s + r.costo_unitario, 0)

  return (
    <>
      <Title level={3}>📊 Datos CD</Title>
      <Row gutter={16} style={{ marginBottom: 12 }}>
        {[
          { label: 'Filas', value: rows.length },
          { label: 'Σ Costo Unitario', value: `$${fmt(sumCU)}` },
          { label: 'Ítems con datos', value: new Set(rows.map(r => r.item_id)).size },
        ].map(m => (
